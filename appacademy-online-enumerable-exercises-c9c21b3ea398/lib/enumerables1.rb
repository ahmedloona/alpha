...
def price_is_right(bids, actual_retail_price)
  bids.reduce(0) do |closest_bid, bid|
    if bid > closest_bid && bid < actual_retail_price
      bid
    else
      closest_bid
    end
...
def num_factors(number)
  (1..number).reduce(0) do |count, candidate|
    (number % candidate == 0) ? count += 1 : count
  end
end
...
def ordered_vowel_word?(word)
  vowels = %w(a e i o u)
  word.each_char.reduce('a') do |biggest_vowel, ch|
    next biggest_vowel if !vowels.include?(ch.downcase)
    if ch.downcase >= biggest_vowel
      ch.downcase
    else
      return false
    end
...
def products_except_me(numbers)
  ...
  numbers.each_with_index.reduce([]) do |result, (current_number, idx)|
    if current_number != 0
      result << product / current_number
    else
      array = numbers[0..idx - 1] + numbers[idx + 1..-1]
