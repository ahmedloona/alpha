...
def caesar_cipher(str, shift)
  ...
  str.each_char do |char|
    ...
    new_idx = (old_idx + shift % letters.count) % letters.count
...
def transpose(matrix)
  result = Array.new(matrix[0].length) {Array.new} #HARD - PASS BY REFERENCE ISSUE
  matrix.each do |row|
...
class Array
  def my_inject(accumulator = nil, &block)
    i = 0
    accumulator, i = self[0], 1 unless accumulator
    while i < length
