  ...
  def bubble_sort!(&prc)
    ...
    until sorted
      ...
      self.each_with_index do |el, idx|
        ...
        comparison = prc.call(el, self[idx + 1])
        if comparison == 1
...
def subwords(word, dictionary)
  dictionary.reduce([]) do |acc, subword|
    (word.include?(subword)) ? acc << subword : acc
  end
end
